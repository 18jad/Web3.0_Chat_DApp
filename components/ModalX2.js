    ...
    <BackDrop onClick={handleClose}>
      <motion.div
        ...
        <div className='absolute top-0  h-16 w-16 m-5 border-purple-700'>
          <Image
            className=''
            src={`https://avatars.dicebear.com/api/bottts/${user?.get(
              "username",
            )}.svg`}
            layout='fill'
          />
        ...
        <div className='flex flex-col justify-center items-center gap-6 text-xl text-white font-effect-neon mt-16'>
          ...
          <div>Email: {user?.get("email")}</div>
          <div>Eth: {user?.get("ethAddress")}</div>
          <div>Verified: {user?.get("emailVerified") ? "true" : "false"}</div>
          <ByMoralis
            variant='dark'
