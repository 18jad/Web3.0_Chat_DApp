...
export default function SendMessage({ endOfMessagesRef }) {
  ...
  return (
    <div className='flex w-11/12 border-4 border-purple-700 shadow-xl shadow-gray-800 bg-black opacity-60 fixed bottom-10 px-6 py-3 max-w-2xl rounded-full'>
      <div
        className={`text-white absolute -top-[450px] ${
          emoji ? "block" : "hidden"
        }`}>
      ...
      <form className='flex items-center w-full'>
        <input
          type='text'
          className='outline-none  flex-grow bg-transparent text-white placeholder-white pl-5 text-xl'
        ...
        <button
          type='submit'
          onClick={sendMessage}
          className='font-bold text-white opacity-60'>
          Send
        </button>
      </form>
    </div>
