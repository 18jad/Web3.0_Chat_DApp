...
function Login() {
  ...
  const { authenticate, isAuthenticating, authError, hasAuthError } =
    useMoralis();
  const item = {
  ...
  if (!loading)
    return (
      <div className='flex justify-center items-center h-screen bg-[#333] flex-col text-white space-y-6'>
        ...
        <RingLoader color={"#C715F3"} size={200} />
        <h1 className='text-[45px] text-[#C715F3] logo select-none'>
          METAVERSE
        </h1>
        <BarLoader color={"#C715F3"} size={200} />
      </div>
    );
  return (
    <div className='Main'>
      <Head>
        ...
        <link rel='preconnect' href='https://fonts.gstatic.com' crossorigin />
        <link
          href='https://fonts.googleapis.com/css?family=Kanit&display=swap&effect=neon|3d-float|emboss'
          rel='stylesheet'
        />
      </Head>
      <div className='relative z-50'>
        ...
        />
        {authError ? alert(authError.message) : null}
        <motion.div
          variants={item}
          initial='hidden'
          animate='visible'
          ...
          {!isAuthenticating ? (
            <div className=''>
              <div className='flex flex-col items-center justify-center mainBox logBox relative'>
                <h1 className='mt-[-60px] text-4xl  text-white titleBox w-full text-center p-4 	font-effect-neon'>
                ...
                {hide ? (
                  <button
                    className='btnA2 font-effect-3d-float p-2 rounded-full cursor-pointer w-full mt-2'
                    onClick={() => {
                      setLogInput(false);
                      setHide(false);
                    }}>
